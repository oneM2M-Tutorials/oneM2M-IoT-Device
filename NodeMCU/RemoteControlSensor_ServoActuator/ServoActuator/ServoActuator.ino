...
int SERVO_PIN = D4;  // PIN for the Servo Motor
...
void init_IO() {
  ...
  myservo.attach(SERVO_PIN);  // attaching the servo with the corresponding pin
}
void task_IO() {
}
...
void handle_HTTPMessage(){
  if (context != "") {
      if (context == "ServoActuator") {
          command_servo(command);
          task_servo(command);
      }
      else
...
void init_servo() {
  String initialDescription = "Name = ServoActuator\t"
                              "Location = Home\t";
  String initialData = "void";
  originator = "Cae-ServoActuator";
  registerModule("ServoActuator", true, initialDescription, initialData);
}
void task_servo(String cmd) {
  originator = "Cae-ServoActuator";
  createCI("ServoActuator", DATA_CNT_NAME, cmd);  
}
void command_servo(String cmd) {
  // We show the received command on the LCD for debug purposes
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("MSG Received => ");
  lcd.setCursor(0, 1);

  // Apply the received command on the servo
  if (cmd.equals("POS = 0 deg")) {
    myservo.write(0);
    lcd.print("Snowy !"); 
  }
  if (cmd.equals("POS = 45 deg")) { 
    myservo.write(45);
    lcd.print("Stormy !"); 
  }
  if (cmd.equals("POS = 90 deg")){ 
    myservo.write(90);
    lcd.print("Rainy !"); 
  }
  if (cmd.equals("POS = 135 deg")) {
    myservo.write(135);
    lcd.print("Cloudy !"); 
  }
  if (cmd.equals("POS = 180 deg")) {
    myservo.write(180);
    lcd.print("Sunny !"); 
  }

  // wait some time and clear the LCD
  delay(1000);
  lcd.clear();
}
